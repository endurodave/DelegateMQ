...
- [Deadline Monitoring — `DeadlineSubscription<T>`](#deadline-monitoring--deadlinesubscriptiont)
  - [Behaviour](#behaviour)
  - [`Timer::ProcessTimers()` requirement](#timerprocesstimers-requirement)
...
- **QoS (Quality of Service)**: Per-subscription configuration for topic behavior — see [Quality of Service (QoS)](#quality-of-service-qos).
...
QoS options are set per-subscription via the `dmq::QoS` struct and passed as the last argument to `DataBus::Subscribe`. Options compose freely — any combination of the fields below is valid.
...
auto conn = dmq::DataBus::Subscribe<SensorData>("sensor/temp", handler, &thread, qos);
```
...
- **Cache depth**: exactly one value per topic. The DataBus stores only the latest snapshot.
...
- **Memory**: the DataBus only caches values for topics where at least one subscriber has requested LVC (`lastValueCache = true`).
...
auto conn = dmq::DataBus::Subscribe<float>("sensor/temp", handler, nullptr, qos);
...
auto conn = dmq::DataBus::Subscribe<ImuData>("sensor/imu", handler, &thread, qos);
...
## Deadline Monitoring — `DeadlineSubscription<T>`
...
`dmq::DeadlineSubscription<T>` wraps a `DataBus::Subscribe` call with a `Timer`. Every incoming message resets the timer. If the timer fires before the next message arrives, the `onMissed` callback is invoked. Both callbacks are dispatched to the same optional worker thread.
...
The object is non-copyable and non-movable. Declare it as a class member or use `unique_ptr<DeadlineSubscription<T>>` for heap allocation. When it is destroyed, the DataBus connection and the timer are both cleaned up automatically — no dangling callbacks.
...
- **Thread dispatch** — with a thread argument, both the data callback and `onMissed` are dispatched to that thread. Without one, the data callback fires on the publisher's thread and `onMissed` fires on the `Timer::ProcessTimers()` thread.

### `Timer::ProcessTimers()` requirement

The deadline fires only when `Timer::ProcessTimers()` is called. On platforms where a `Thread` with a watchdog is already running, this is driven automatically. On bare-metal targets, call `ProcessTimers()` from the main super-loop or a SysTick handler. If `ProcessTimers()` is never called, `onMissed` silently never fires.
...
dmq::DeadlineSubscription<ImuData> m_watch{
    "sensor/imu",
