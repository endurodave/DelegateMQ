...
namespace Example
{
    void DataBusExample()
    {
        ...
        // 2. Different types on different topics
        {
            ...
            auto conn = dmq::databus::DataBus::Subscribe<string>("system/status", [](string status) {
                cout << "System status: " << status << endl;
            });

            // Publish to "system/status" topic
            dmq::databus::DataBus::Publish<string>("system/status", "OK");
        }
        
        // 3. Multiple subscribers
        {
            auto conn1 = dmq::databus::DataBus::Subscribe<int>("count", [](int c) { cout << "Sub 1 count: " << c << endl; });
            auto conn2 = dmq::databus::DataBus::Subscribe<int>("count", [](int c) { cout << "Sub 2 count: " << c << endl; });
        ...
        // 4. Asynchronous Subscription on a worker thread
        {
            ...
            auto conn = dmq::databus::DataBus::Subscribe<int>("async/topic", [&](int val) {
                cout << "Async callback received value: " << val << " on thread: " << Thread::GetCurrentThreadId() << endl;
                received = true;
            }, &workerThread);

            // Publish to "async/topic"
            dmq::databus::DataBus::Publish<int>("async/topic", 123);
